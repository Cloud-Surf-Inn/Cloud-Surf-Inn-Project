...
public class roomMatcher
{
	public static int match(String[][] argDatabase, String occupants, String kitchen, String jacuzzi, String bidet) {
	   ...
	   for(int j = 0; j < numRooms; j++)
	   {
		...
		int checkOccupants;
		int highestOccupants = 0;
		
		for(int i = 0; i < numRooms; i++)
		{
			if (dataFake[i][2] > highestOccupants)
			highestOccupants = dataFake[i][2];
		}
		
		
	    pref1 = occupants; 
	    checkOccupants = Integer.parseInt(occupants);
	    if (checkOccupants > highestOccupants)
	    {
			
			TooManyBeds windowTest = new TooManyBeds();
			windowTest.runWindow();
	    }
	    pref2 = kitchen;
	    pref3 = jacuzzi;
	    pref4 = bidet;
